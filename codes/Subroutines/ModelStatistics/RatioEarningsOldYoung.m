function RatioEarningsOldYoung=RatioEarningsOldYoung(Nsim, StationaryDist, Policy, Phi_aprimeMatrix, n_d,n_a,n_z, d_grid, Gamma, e,w,J)

% Life-cycle profile of earnings
%  (avg. earnings of HH aged 45 to 49)/(avg. earnings of HH aged 25 to 29)
...
% Only people who work for all 30 periods are considered in this sample.
...
% for 30 years are considered (if they retire they are deleted from this
% sample).
IndividualEarnings_Young=zeros(K*Nsim,1); % Age 25-29
IndividualEarnings_Old=zeros(K*Nsim,1);   % Age 45-49
cumsumpi_s=cumsum(Gamma(1:J,:),2); % Rows have to add to one anyway

pi_temp=cumsum(Gamma(J+1:2*J,1:J),2);
...
parfor i=1:K*Nsim 
    ...
    if TheyRetired==0
        IndividualEarnings_Young(i)=mean(YoungEarnings(5:9)); % YoungEarnings(5:9) corresponds to ages 25-29
        IndividualEarnings_Old(i)=mean(OldEarnings(5:9)); % OldEarnings(5:9) corresponds to ages 45-49
%         disp('Got one!')
    end
end
...
SizeIndividualEarnings_Old=size(IndividualEarnings_Old);
disp(' ');
fprintf('Number of sample points for ratio of earnings old-to-young: %.f \n',SizeIndividualEarnings_Old(1))
...
% 50 year olds may be retired. We do not wish to allow for this when
% calculating this statistic. So we only consider people who make it through
% the 30 periods without retiring.
